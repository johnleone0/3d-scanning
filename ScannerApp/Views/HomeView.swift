    ...
    private var modelsList: some View {
        List {
            ForEach(appState.scannedModels) { model in
                NavigationLink(destination: ModelPreviewView(model: model)) {
    ...
    private var noLiDAROverlay: some View {
        VStack(spacing: 16) {
            ...
            Text("LiDAR Not Available")
                .font(.title2.bold())
            Text("This app requires an iPhone or iPad with a LiDAR sensor (iPhone 12 Pro and later).")
                .font(.subheadline)
                .foregroundStyle(.secondary)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(model.name)
                .font(.headline)
            HStack(spacing: 12) {
                Label("\(model.vertexCount.formatted()) vertices", systemImage: "circle.grid.3x3")
